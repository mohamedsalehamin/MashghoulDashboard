<?php

namespace App\Policies;

use App\Models\User;
use App\ContentModule\Models\JoinRequest;
use Illuminate\Auth\Access\HandlesAuthorization;

class JoinRequestPolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the user can view any models.
     *
     * @param User $user
     * @return bool
     */
    public function viewAny(User $user): bool
    {
        return $user->can('ViewAny:JoinRequest');
    }

    /**
     * Determine whether the user can view the model.
     *
     * @param User $user
     * @param JoinRequest $joinRequest
     * @return bool
     */
    public function view(User $user, JoinRequest $joinRequest): bool
    {
        return $user->can('View:JoinRequest');
    }

    /**
     * Determine whether the user can create models.
     *
     * @param User $user
     * @return bool
     */
    public function create(User $user): bool
    {
        return $user->can('Create:JoinRequest');
    }

    /**
     * Determine whether the user can update the model.
     *
     * @param User $user
     * @param JoinRequest $joinRequest
     * @return bool
     */
    public function update(User $user, JoinRequest $joinRequest): bool
    {
        return $user->can('Update:JoinRequest');
    }

    /**
     * Determine whether the user can delete the model.
     *
     * @param User $user
     * @param JoinRequest $joinRequest
     * @return bool
     */
    public function delete(User $user, JoinRequest $joinRequest): bool
    {
        return $user->can('Delete:JoinRequest');
    }

    /**
     * Determine whether the user can bulk delete.
     *
     * @param User $user
     * @return bool
     */
    public function deleteAny(User $user): bool
    {
        return $user->can('DeleteAny:JoinRequest');
    }

    /**
     * Determine whether the user can permanently delete.
     *
     * @param User $user
     * @param JoinRequest $joinRequest
     * @return bool
     */
    public function forceDelete(User $user, JoinRequest $joinRequest): bool
    {
        return $user->can('ForceDelete:JoinRequest');
    }

    /**
     * Determine whether the user can permanently bulk delete.
     *
     * @param User $user
     * @return bool
     */
    public function forceDeleteAny(User $user): bool
    {
        return $user->can('ForceDeleteAny:JoinRequest');
    }

    /**
     * Determine whether the user can restore.
     *
     * @param User $user
     * @param JoinRequest $joinRequest
     * @return bool
     */
    public function restore(User $user, JoinRequest $joinRequest): bool
    {
        return $user->can('Restore:JoinRequest');
    }

    /**
     * Determine whether the user can bulk restore.
     *
     * @param User $user
     * @return bool
     */
    public function restoreAny(User $user): bool
    {
        return $user->can('RestoreAny:JoinRequest');
    }

    /**
     * Determine whether the user can replicate.
     *
     * @param User $user
     * @param JoinRequest $joinRequest
     * @return bool
     */
    public function replicate(User $user, JoinRequest $joinRequest): bool
    {
        return $user->can('Replicate:JoinRequest');
    }

    /**
     * Determine whether the user can reorder.
     *
     * @param User $user
     * @return bool
     */
    public function reorder(User $user): bool
    {
        return $user->can('Reorder:JoinRequest');
    }

}
