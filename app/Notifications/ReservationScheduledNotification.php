...
class ReservationScheduledNotification extends Notification {
    use Queueable;
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ...
            ->doctorMessage('panel.notifications.doctorreservationschedulednotification', ['id' => $this->reservation->id])
            ->parse();
    }
    ...
    public function getBody($notifiable) {
        ...
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.reservationschedulednotification', ['id' => $this->reservation->id])
            ->doctorMessage('panel.notifications.doctorreservationschedulednotificationbody', ['id' => $this->reservation->id])
            ->parse();
    }
}
