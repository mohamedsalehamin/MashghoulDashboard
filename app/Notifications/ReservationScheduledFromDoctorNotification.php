...
class ReservationScheduledFromDoctorNotification extends Notification {
    use Queueable;
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.reservationscheduledfromdoctornotification', ['id' => $this->reservation->id])
            ->doctorMessage('panel.notifications.doctorreservationscheduledfromdoctornotification', ['id' => $this->reservation->id])
            ->parse();
    }
    ...
    public function getBody($notifiable) {
        ...
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.reservationscheduledfromdoctornotificationbody', ['id' => $this->reservation->id])
            ->doctorMessage('panel.notifications.doctorreservationscheduledfromdoctornotificationbody', ['id' => $this->reservation->id])
            ->parse();
    }
}
