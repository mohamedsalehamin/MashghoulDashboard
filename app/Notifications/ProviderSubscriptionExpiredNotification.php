...
class ProviderSubscriptionExpiredNotification extends Notification
{
    use Queueable;
    ...
    public function getTitle($notifiable): array
    {
        return NotificationMessageParser::init($notifiable)
            ->doctorMessage('panel.notifications.provider_subscription_expired')
            ->parse();
    }
    ...
    {
        return NotificationMessageParser::init($notifiable)
            ->doctorMessage('panel.notifications.provider_subscription_expired_body')
            ->parse();
    }
}
