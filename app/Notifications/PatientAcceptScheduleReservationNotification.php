...
class PatientAcceptScheduleReservationNotification extends Notification {
    use Queueable;
...
//            ->customerMessage('panel.notifications.reservationschedulednotification', ['id' => $this->reservation->id])
            ->doctorMessage('panel.notifications.patientacceptschedulereservationnotification', ['id' => $this->reservation->id])
            ->parse();
    }
    ...
    public function getBody($notifiable) {
        ...
        return NotificationMessageParser::init($notifiable)
//            ->customerMessage('panel.notifications.reservationschedulednotification', ['id' => $this->reservation->id])
            ->doctorMessage('panel.notifications.patientacceptschedulereservationnotificationbody', ['id' => $this->reservation->id])
            ->parse();
    }
}
