    ...
    public static function make() {
        return Filter::make('locations')
            ...
            ->query(function (Builder $query, array $data): Builder {
                return $query
                    ->when($data['country_id'] ?? '', fn(Builder $query, $country_id): Builder => $query->whereHas('city.state', fn(Builder $query) => $query->where('country_id', $country_id)))
                    ->when($data['state_id'] ?? '', fn(Builder $query, $state_id): Builder => $query->whereHas('city', fn(Builder $query) => $query->where('state_id', $state_id)))
                    ->when($data['city_id'] ?? '', fn(Builder $query, $state_id): Builder => $query->where('city_id', $state_id));
            });
    }
}
