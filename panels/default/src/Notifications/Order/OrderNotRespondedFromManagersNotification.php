...
class OrderNotRespondedFromManagersNotification extends Notification {
    use Queueable;
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.order_not_responded', ['id' => $this->order->id])
            ->parse();
    }

    public function getBody($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.order_not_responded_body')
            ->parse();
    }
}
