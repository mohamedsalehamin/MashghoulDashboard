...
class OrderStatusChangedNotification extends Notification {
    ...
    public function getTitle($notifiable) {
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.order_status_changed', ['id' => $this->order->id])
            ->managerMessage('panel.notifications.order_status_changed', ['id' => $this->order->id])
    ...
    public function getBody($notifiable) {
        ...
        return NotificationMessageParser::init($notifiable)
            ->customerMessage('panel.notifications.order_status_changed_body', ['status' => $this->order->status->getLabel(),'ID' => $this->order->order_number])
            ->managerMessage('panel.notifications.order_status_changed_body', ['status' => $this->order->status->getLabel(),'ID' => $this->order->order_number])
            ->parse();
    }
}
